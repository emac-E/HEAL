...
└── heal.py fix-pattern <PATTERN_ID>
...
```bash
# Fix entire pattern
python scripts/heal.py fix-pattern EOL_CONTAINER_COMPATIBILITY \
    --max-iterations 10 \
    --threshold 0.80  # Pass if 80% of tickets fixed - ? is this adequate? can it do better?

# List available patterns
python scripts/heal.py list-patterns

# Show pattern details
python scripts/heal.py show-pattern EOL_CONTAINER_COMPATIBILITY
```
...
python scripts/heal.py fix-pattern EOL_CONTAINER_COMPATIBILITY --threshold 0.8
```
...
python scripts/heal.py validate-pattern EOL_CONTAINER_COMPATIBILITY
...
- [ ] Add `fix-pattern` command to heal.py
...
python scripts/heal.py fix-patterns --auto-prioritize
...
python scripts/heal.py pattern-dashboard
# Output:
...
python scripts/heal.py fix-patterns --resolve-dependencies
```
