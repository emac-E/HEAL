...
**Files Fixed:**
- ✅ `scripts/heal.py` - TIER_MODELS config
...
The system has **full bidirectional escalation and de-escalation** implemented in `scripts/heal.py`:
...
#### Escalation Logic (heal.py:4131-4156)
...
**NEW: Down to Cheaper Models (heal.py:should_deescalate_model)**
...
**Fallback to Sonnet on Opus Failure (heal.py:4302-4314)**
...
```python
# Don't escalate to Opus if it has already failed (heal.py:4464-4468)
