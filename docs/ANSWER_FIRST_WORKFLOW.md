...
uv run scripts/heal.py diagnose CUSTOMER-BUG-123
```
...
🔍 DIAGNOSIS: WRONG DOCUMENTS RETRIEVED
   ...
   💡 Next command:
      uv run scripts/heal.py bootstrap CUSTOMER-BUG-123 --yolo
```
...
```bash
uv run scripts/heal.py bootstrap CUSTOMER-BUG-123 --yolo --max-iterations 20
```
...
uv run scripts/heal.py validate
...
**Step 2: Run batch**
```bash
uv run scripts/heal.py fix --ticket-file customer_bugs.txt --yolo --max-iterations 20
```
...
uv run scripts/heal.py diagnose CUSTOMER-BUG-123 --use-existing
...
uv run scripts/heal.py bootstrap TICKET-ID --auto-select-threshold 50.0

# Less selective (lower-scoring docs too)
uv run scripts/heal.py bootstrap TICKET-ID --auto-select-threshold 5.0

# Manual selection (asks user)
uv run scripts/heal.py bootstrap TICKET-ID --auto-select-threshold 0.0
```

### Customize Verification Prompt

Edit `heal.py:check_answer_in_retrieved_docs()` to adjust verification criteria.
...
uv run scripts/heal.py bootstrap TICKET-ID --yolo

# Later: Refine retrieval with known docs
uv run scripts/heal.py fix TICKET-ID --yolo --max-iterations 5
```
...
uv run scripts/heal.py bootstrap TICKET-1 --yolo --max-iterations 5

# Then scale up
uv run scripts/heal.py fix TICKET-1 TICKET-2 TICKET-3 --yolo --max-iterations 20
```
