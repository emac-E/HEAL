...
│                                                                 │
│  Entry Points:                                                  │
│    - python scripts/heal.py diagnose RSPEED-XXXX      │
│    - python scripts/heal.py fix RSPEED-XXXX           │
│    - python scripts/heal.py validate                  │
│                                                                 │
│  Outputs:                                                       │
...
python scripts/heal.py diagnose RSPEED-2482 \
  --output-format json \
  --output-file diagnosis.json

# Fix a ticket (creates worktree with proposed changes)
python scripts/heal.py fix RSPEED-2482 \
  --max-iterations 10 \
  --output-format json \
...
```bash
python scripts/heal.py diagnose RSPEED-2482 --output-format json
```
...
```bash
python scripts/heal.py fix RSPEED-2482 --output-worktree
```
...
python scripts/heal.py jira list \
  --project RSPEED \
  --status "Open" \
...
jq -r '.tickets[].key' "${LOG_DIR}/tickets_${DATE}.json" | while read -r ticket_id; do
  ...
  python scripts/heal.py diagnose "$ticket_id" \
    --output-format json \
...
jq -r '.tickets[].key' "${LOG_DIR}/priority_tickets_${DATE}.json" | while read -r ticket_id; do
  ...
  if python scripts/heal.py fix "$ticket_id" \
      --max-iterations 10 \
      --validate-cla-tests \
      --output-worktree \
      --output-format json \
    ...
    # Update JIRA
    python "$WORK_DIR/scripts/heal.py" jira comment "$ticket_id" \
    ...
    python scripts/heal.py jira comment "$ticket_id" \
...
```
# Tickets processed
heal_tickets_total{status="success"} 42
heal_tickets_total{status="failure"} 3

# Diagnosis results
heal_diagnosis_type{type="retrieval_problem"} 25
heal_diagnosis_type{type="answer_problem"} 10
heal_diagnosis_type{type="passing"} 7

# LLM costs
heal_llm_cost_usd{model="haiku"} 0.05
heal_llm_cost_usd{model="sonnet"} 2.10

# Evaluation duration
heal_eval_duration_seconds{mode="retrieval_only"} 30
heal_eval_duration_seconds{mode="full"} 180
```
...
python scripts/heal.py health-check
          ...
          - name: agent
            ...
            - |
              ...
              while read ticket; do
                python scripts/heal.py diagnose "$ticket" \
                  --output-format json \
                  --post-to-jira
              done
            volumeMounts:
