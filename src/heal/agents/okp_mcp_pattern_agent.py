...
# Define repo root for file paths in main()
...
class PatternEvaluationResult:
    ...
    def __post_init__(self):
        ...
        self.pass_count = sum(1 for r in self.per_ticket_results.values() if r.is_passing)
        ...
        self.avg_url_f1 = (
            sum(metrics_lists["url_f1"]) / len(metrics_lists["url_f1"])
            if metrics_lists["url_f1"]
            else None
        )
        self.avg_context_relevance = (
            sum(metrics_lists["context_relevance"]) / len(metrics_lists["context_relevance"])
            if metrics_lists["context_relevance"]
            else None
        )
        self.avg_context_precision = (
            sum(metrics_lists["context_precision"]) / len(metrics_lists["context_precision"])
            if metrics_lists["context_precision"]
            else None
        )
        self.avg_answer_correctness = (
            sum(metrics_lists["answer_correctness"]) / len(metrics_lists["answer_correctness"])
            if metrics_lists["answer_correctness"]
            else None
        )
...
class OkpMcpPatternAgent(OkpMcpAgent):
    ...
    def get_pattern_tickets(self, pattern_id: str) -> List[dict]:
        ...
        pattern_tickets = [
            t for t in self.tickets_with_patterns if t.get("pattern_id") == pattern_id
        ]
    ...
    def evaluate_pattern(
        ...
        for i, ticket in enumerate(tickets, 1):
            ...
            try:
                ...
                status = "✅ PASS" if result.is_answer_good_enough else "❌ FAIL"
                print(f"  {status}")
        ...
        if pattern_result.pattern_fixed:
            print(
                f"✅ PATTERN FIXED (pass rate {pattern_result.pass_rate:.1%} >= threshold {threshold:.0%})"
            )
        else:
            print(
                f"❌ PATTERN NOT FIXED (pass rate {pattern_result.pass_rate:.1%} < threshold {threshold:.0%})"
            )
...
def main():
    ...
    subparsers.add_parser("list-patterns", help="List all discovered patterns")

    # show-pattern command
    show_parser = subparsers.add_parser(
        "show-pattern", help="Show detailed information about a pattern"
    )
    show_parser.add_argument("pattern_id", help="Pattern ID")

    # fix-pattern command
    fix_parser = subparsers.add_parser("fix-pattern", help="Fix all tickets in a pattern")
    fix_parser.add_argument("pattern_id", help="Pattern ID")
    fix_parser.add_argument(
        "--max-iterations", type=int, default=10, help="Maximum fix iterations (default: 10)"
    )
    fix_parser.add_argument(
        "--threshold",
        type=float,
    ...
    validate_parser = subparsers.add_parser(
        "validate-pattern", help="Validate all tickets in a pattern"
    )
