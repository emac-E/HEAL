...
class TicketQuery(BaseModel):
    ...
    expected_response: str = Field(
        description="The correct answer that the LLM should give (extracted from ticket or marked as TODO if not found)"
    )
...
async def fetch_jira_tickets_with_agent(
    ...
    """
    print("🔍 Fetching JIRA tickets via Claude Agent SDK...")
    print(f"   Query: {jql}")
    print(f"   Limit: {limit}")
...
"""
    ...
    try:
        ...
        print("⚠️  Could not parse JIRA response - no 'issues' field found")
        print(f"   Response text: {text[:500]}...")
        return []

    except Exception as e:
        print(f"❌ Error fetching JIRA tickets: {e}")
        print("   Make sure the mcp-atlassian MCP server is configured and authenticated")
        return []
...
async def generate_test_config(
    ...
    for issue in issues:
        ...
        print(
            f"   ✓ Expected response: {'Found' if has_expected else 'Not found (needs SME input)'}"
        )
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    tickets_with_expected = sum(
        1 for c in conversations if not c["turns"][0]["expected_response"].startswith("TODO:")
    )
    ...
    print(
        f"   ⚠️  Need SME input: {tickets_need_sme}" + (f" → {sme_file.name}" if sme_file else "")
    )

    if tickets_with_expected > 0:
        print(f"\n🚀 Next steps for ready tickets ({ready_file}):")
        print("  1. Batch process with bootstrap to discover URLs:")
        print(
            f"     for ticket in $(grep 'conversation_group_id:' {ready_file} | awk '{{print $2}}'); do"
        )
        print("       python scripts/okp_mcp_agent.py bootstrap ${ticket//_/-} --yolo")
        print("     done")
    ...
    if sme_file:
        ...
        print("  1. Assign to SMEs to fill in expected_response fields")
        print(f"  2. Once filled in, move tickets to {ready_file}")
        print("  3. Then run bootstrap as above")
