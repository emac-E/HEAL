...
async def test_te001_sonnet_baseline():
    ...
    prompt = (
        "You are a code analyst. Analyze this config and respond with 'ANALYZED' if you understand."
    )
...
async def test_te002_opus_direct():
    ...
    try:
        ...
        if "Command failed with exit code 1" in error_msg:
            print("   ⚠️  This is the 'exit code 1' error from heal!")
...
Expected docs not ranking well. What should change?"""
        ...
        if complexity == "COMPLEX":
            print("  Problem classified as COMPLEX - escalating to Opus")
...
""")
    ...
    prompt = """You are a Solr optimization expert.
...
```"""
    ...
    try:
        ...
        print("🔍 Calling Opus with file editing enabled...")
        print(f"   Model: {TIER_MODELS['complex']}")
        print("   Tools: Read, Edit, Glob, Grep")
        ...
        print("✅ Opus with file editing succeeded")
