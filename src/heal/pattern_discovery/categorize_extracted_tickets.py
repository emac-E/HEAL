...
def categorize_tickets(
    ...
    report: Dict[str, Any] = {
        ...
        "categories": {
            "verified": {
                ...
                "percentage": (
                    round(len(categorized["verified"]) / total_tickets * 100, 1)
                    if total_tickets > 0
                    else 0
                ),
                "description": "Tickets with verified ground truth from documentation",
            ...
            "no_ground_truth": {
                ...
                "percentage": (
                    round(len(categorized["no_ground_truth"]) / total_tickets * 100, 1)
                    if total_tickets > 0
                    else 0
                ),
                "description": "Tickets without ground truth - missing documentation or out of scope",
            ...
            "failed": {
                ...
                "percentage": (
                    round(len(categorized["failed"]) / total_tickets * 100, 1)
                    if total_tickets > 0
                    else 0
                ),
                "description": "Tickets with extraction errors or malformed entries",
    ...
    print()
    print(
        f"✅ Verified: {len(categorized['verified'])} ({report['categories']['verified']['percentage']}%)"
    )
    print("   → Can be fixed via search optimization")
    print()
    print(
        f"⚠️  No ground truth: {len(categorized['no_ground_truth'])} ({report['categories']['no_ground_truth']['percentage']}%)"
    )
    ...
    print()
    print(
        f"❌ Failed: {len(categorized['failed'])} ({report['categories']['failed']['percentage']}%)"
    )
    ...
    print("   python scripts/discover_ticket_patterns.py \\")
    print(f"       --input {output_verified} \\")
    print("       --output-tagged tickets_with_patterns.yaml \\")
    print("       --output-report patterns_report.json")
    print()
...
def main():
    ...
    parser.add_argument("--input", type=Path, required=True, help="Path to extracted_tickets.yaml")
    parser.add_argument(
        "--output-verified", type=Path, required=True, help="Path for verified tickets output"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-failed", type=Path, required=True, help="Path for failed tickets output"
    )
    parser.add_argument(
        "--report", type=Path, required=True, help="Path for categorization report JSON"
    )
