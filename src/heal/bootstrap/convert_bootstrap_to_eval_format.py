...
def load_inputs(tickets_file: Path, patterns_file: Path, tagged_file: Path) -> tuple:
...
def convert_ticket_to_eval_format(ticket: dict, pattern_id: Optional[str] = None) -> dict:
    ...
    has_ground_truth = (
        expected_response
        and expected_response.strip()
        and not expected_response.upper().startswith("N/A")
        and not expected_response.upper().startswith("TODO:")
        and "not rhel-related" not in expected_response.lower()
        and "not a rhel support question" not in expected_response.lower()
        and "no documentation found" not in expected_response.lower()
    )

    # If no ground truth, leave blank for human to fill
    if not has_ground_truth:
        notes = (
            expected_response if expected_response else "Needs manual review - missing ground truth"
        )
        expected_response = ""
        sources = []
    else:
    ...
    # Build conversation
    conversation = {"conversation_group_id": ticket_key, "turns": [turn]}
...
def group_tickets_by_pattern(
    tickets: List[dict], tagged_tickets: List[dict]
) -> Dict[str, List[dict]]:
...
def write_pattern_yaml(
    ...
    tickets: List[dict],
    output_dir: Path,
    patterns_data: Optional[Dict[Any, Any]] = None,
):
    ...
    for ticket in tickets:
        conv = convert_ticket_to_eval_format(
            ticket, pattern_id if pattern_id not in ["UNGROUPED", "NO_GROUND_TRUTH"] else None
        )
...
def main():
    ...
    parser.add_argument(
        "--tickets", type=Path, required=True, help="Path to extracted_tickets.yaml"
    )
    parser.add_argument("--patterns", type=Path, required=True, help="Path to patterns_report.json")
    parser.add_argument(
        "--tagged", type=Path, required=True, help="Path to tickets_with_patterns.yaml"
    )
    parser.add_argument(
        "--output-dir", type=Path, required=True, help="Output directory for pattern YAMLs"
    )
    ...
    # Load inputs
    tickets, patterns_data, tagged_tickets = load_inputs(args.tickets, args.patterns, args.tagged)
    ...
    print(
        f"  Patterns: {len([k for k in grouped.keys() if k not in ['UNGROUPED', 'NO_GROUND_TRUTH']])}"
    )
    ...
    print("       --config config/system_okp_mcp_agent.yaml \\")
    print("       --data config/patterns/AUTHENTICATION_ISSUES.yaml \\")
    print("       --metrics ragas:context_relevance,custom:answer_correctness")
    print()
