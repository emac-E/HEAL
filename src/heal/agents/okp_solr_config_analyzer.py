...
class SolrConfigAnalyzer:
    ...
    def parse_current_config(self) -> Dict:
        ...
        boost_match = re.search(
            r"_EXTRACTION_BOOST_KEYWORDS = frozenset\(\s*\[(.*?)\]", content, re.DOTALL
        )
        if boost_match:
        ...
        demote_match = re.search(
            r"_EXTRACTION_DEMOTE_RHV = frozenset\(\s*\[(.*?)\]", content, re.DOTALL
        )
        if demote_match:
    ...
    def get_explain_output(
        self, query: str, doc_ids: Optional[List[str]] = None, num_docs: int = 10
    ) -> Dict:
    ...
    def analyze_ranking_problems(
        self, query: str, expected_urls: List[str], retrieved_urls: List[str]
    ) -> Dict:
    ...
    def search_for_answer_content(
        self, keywords: List[str], expected_response: Optional[str] = None, num_results: int = 10
    ) -> List[Dict]:
        ...
        for term in search_terms[:20]:  # Limit to top 20 terms
            ...
            query_parts.append(
                f'(allTitle:"{escaped_term}" OR abstract:"{escaped_term}" OR body:"{escaped_term}")'
            )
        ...
        try:
            ...
            for doc in docs:
                ...
                url = (
                    f"https://access.redhat.com{uri}" if uri and not uri.startswith("http") else uri
                )
                ...
                results.append(
                    {
                        "url": url,
                        "title": (
                            doc.get("allTitle", ["No title"])[0]
                            if isinstance(doc.get("allTitle"), list)
                            else doc.get("allTitle", "No title")
                        ),
