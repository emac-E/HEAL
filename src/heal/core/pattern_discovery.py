...
class PatternTemplate(BaseModel):
    ...
    pattern_id: str
    response_template: str  # With placeholders: {source_version}, {target_version}, etc.
...
Return JSON array."""
            ...
            try:
                ...
                logger.debug(
                    f"  Classifying batch {i // batch_size + 1}: {len(batch)} tickets, prompt size: {len(full_prompt)} chars"
                )
                ...
                logger.error(f"  Claude SDK classification failed: {type(e).__name__}: {e}")
                ...
                logger.error(
                    f"  Failed classification batch: {i // batch_size + 1}, {len(batch)} tickets"
                )
    ...
    async def _discover_patterns_batch(
        ...
        logger.info(f"  Discovering patterns for {batch_name}: {len(classifications)} tickets")
...
Identify patterns (≥3 tickets per pattern)."""
        ...
        # Call Claude
        full_prompt = f"{system_prompt}\n\n{prompt}"
        logger.debug(f"    Prompt size: {len(full_prompt)} chars, {len(full_prompt.split())} words")
        ...
        try:
            ...
            logger.error(f"    Claude SDK query failed: {type(e).__name__}: {e}")
            ...
            # Log prompt info for debugging
            logger.error(
                f"    Failed batch: {batch_name}, {len(classifications)} tickets, prompt size: {len(full_prompt)} chars"
            )
            ...
            # Restore env var before retry/return
            if saved_google_creds:
                os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = saved_google_creds
            ...
            if retry_count < max_retries:
                ...
                logger.error(f"    Max retries reached for {batch_name}, returning empty patterns")
                return []
        finally:
            # Restore GOOGLE_APPLICATION_CREDENTIALS
            if saved_google_creds:
    ...
    async def discover_patterns(
        ...
        Args:
            ...
            batch_size: Max tickets per Claude SDK call (default: 15)
        ...
        for problem_type, tickets in by_problem_type.items():
            ...
            if len(tickets) <= batch_size:
                ...
                for i in range(0, len(tickets), batch_size):
                    ...
                    patterns = await self._discover_patterns_batch(batch, batch_name=batch_name)
                    all_patterns.extend(patterns)
    ...
    def _merge_overlapping_patterns(self, patterns: list[PatternGroup]) -> list[PatternGroup]:
        ...
        logger.info(f"  Merged {len(patterns)} → {len(unique_patterns)} unique patterns")
