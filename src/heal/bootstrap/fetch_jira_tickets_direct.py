        ...
        for i, line in enumerate(lines):
            if "expected" in line.lower() or "should" in line.lower():
                if i + 1 < len(lines):
                    ...
                    reasoning = "Extracted from description near 'expected' keyword"
                    break
...
def main() -> int:
    ...
    print(
        f"   Ready for bootstrap: {len([c for c in conversations if not c['turns'][0]['expected_response'].startswith('TODO:')])} → {ready_file}"
    )
    if sme_file:
        print(
            f"   Need SME input: {len([c for c in conversations if c['turns'][0]['expected_response'].startswith('TODO:')])} → {sme_file}"
        )
